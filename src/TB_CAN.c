...
void initCAN_0(void)
{
  	 INTC_0.PSR[522].R = 0x8002;  //设置MB0的中断优先级为 2 (0位最低，31位最高)
...
void initCAN_1(void)
{
  	 INTC_0.PSR[535].R = 0x8001;  //设置MB0的中断优先级为 1 (0位最低，31位最高)
  	 ...
  	 CAN_1.CTRL1.R = 0x04DB0086;  //数据传输速率的设置
  	 	 	 	 	 	 	 	  /* CAN bus: 40 MHz clksrc, 500K bps with 16 tq */
  	 	 	 	 	 	 	 	  /* PRESDIV+1 = Fclksrc/Ftq = 40 MHz/8MHz = 5 */
     	 	 	 	 	 	 	  /*    so PRESDIV = 4 */
  	 	 	 	 	 	 	 	  /* PSEG2 = Phase_Seg2 - 1 = 4 - 1 = 3 */
  	...
  	#if LOOPBACK   					  //为1 时，是环回模式
  	  CAN_1.CTRL1.B.LPB = 1;		  //使能环回模式
  	  CAN_1.MB[4].CS.B.CODE = 0;      //设置MB[4]位未激活状态
  	  CAN_1.MB[4].CS.B.IDE = 0;       //IDE为0，说明是标准帧;为1，说明是扩展帧
  	  CAN_1.MB[4].ID.B.ID_STD = 0x666;//结合RXMGMASK完成过滤作用
  	  CAN_1.MB[4].CS.B.CODE = 4;      //CODE写入4，将MB[4]的状态设置为Empty
  	  CAN_1.RXMGMASK.R = 0x00000000;  //全局接收过滤设置
  	#endif

  	  CAN_1.MB[0].CS.B.CODE = 0;	  //设置MB[0]位未激活状态
  	  CAN_1.MB[0].CS.B.IDE = 0;       //IDE为0，说明是标准帧;为1，说明是扩展帧
  	  CAN_1.MB[0].ID.B.ID_STD = 0x555;//该寄存器用于设置对应的ID
  	  CAN_1.MB[0].CS.B.CODE = 4;      //CODE写入4，将MB[0]的状态设置为Empty
  	  //全局过滤器，为0时不过滤对应位，为1时过滤，0x1FFFFFFF为全部过滤，即必须完全一样
  	  //标准帧时，RXMGMASK的高十一位，对应ID_STD的十一位，1过滤，0不过滤
  	  CAN_1.RXMGMASK.R = 0x00000000;   //当前状态为*全*不*过滤*
      ...
      //引脚配置，PB1为输入，CAN的RXD
  	  ...
  	  CAN_1.IMASK1.R = 0x00000001;    //中断使能  MB0
}
void initCAN_2(void)
{
  	 INTC_0.PSR[548].R = 0x8002;  //设置MB0的中断优先级为 2 (0位最低，31位最高)
  	 ...
  	 CAN_2.CTRL1.R = 0x04DB0086;  //数据传输速率的设置
  	 	 	 	 	 	 	 	  /* CAN bus: 40 MHz clksrc, 500K bps with 16 tq */
  	 	 	 	 	 	 	 	  /* PRESDIV+1 = Fclksrc/Ftq = 40 MHz/8MHz = 5 */
     	 	 	 	 	 	 	  /*    so PRESDIV = 4 */
  	 	 	 	 	 	 	 	  /* PSEG2 = Phase_Seg2 - 1 = 4 - 1 = 3 */
  	...
  	#if LOOPBACK   					  //为1 时，是环回模式
  	  CAN_2.CTRL1.B.LPB = 1;		  //使能环回模式
  	  CAN_2.MB[4].CS.B.CODE = 0;      //设置MB[4]位未激活状态
  	  CAN_2.MB[4].CS.B.IDE = 0;       //IDE为0，说明是标准帧;为1，说明是扩展帧
  	  CAN_2.MB[4].ID.B.ID_STD = 0x666;//结合RXMGMASK完成过滤作用
  	  CAN_2.MB[4].CS.B.CODE = 4;      //CODE写入4，将MB[4]的状态设置为Empty
  	  CAN_2.RXMGMASK.R = 0x00000000;  //全局接收过滤设置
  	#endif

  	  CAN_2.MB[0].CS.B.CODE = 0;	  //设置MB[0]位未激活状态
  	  CAN_2.MB[0].CS.B.IDE = 0;       //IDE为0，说明是标准帧;为1，说明是扩展帧
  	  CAN_2.MB[0].ID.B.ID_STD = 0x555;//该寄存器用于设置对应的ID
  	  CAN_2.MB[0].CS.B.CODE = 4;      //CODE写入4，将MB[0]的状态设置为Empty
  	  //全局过滤器，为0时不过滤对应位，为1时过滤，0x1FFFFFFF为全部过滤，即必须完全一样
  	  //标准帧时，RXMGMASK的高十一位，对应ID_STD的十一位，1过滤，0不过滤
  	  CAN_2.RXMGMASK.R = 0x00000000;   //当前状态为*全*不*过滤*

  	  //引脚配置，PF14为输出，CAN2的TXD
      ...
      //引脚配置，PF15为输入，CAN的RXD
  	  ...
  	  CAN_2.IMASK1.R = 0x00000001;    //中断使能  MB0
}
...
{
	uint8_t	i;
	CAN_1.MB[0].CS.B.IDE = 0;       	 //标准帧
	CAN_1.MB[0].ID.B.ID_STD = 0x666;	 //报文的ID
	CAN_1.MB[0].CS.B.RTR = 0;            /* Data frame, not remote Tx request frame */
 	CAN_1.MB[0].CS.B.DLC = Data_length ; //要发送的长度
 	for (i=0; i<Data_length; i++)
	{
 		//注意:DATA.B[i]该寄存器最多可以存8个字节的数据
...
{
	uint8_t	i;
	CAN_2.MB[0].CS.B.IDE = 0;       	 //标准帧
	CAN_2.MB[0].ID.B.ID_STD = 0x666;	 //报文的ID
	CAN_2.MB[0].CS.B.RTR = 0;            /* Data frame, not remote Tx request frame */
 	CAN_2.MB[0].CS.B.DLC = Data_length ; //要发送的长度
 	for (i=0; i<Data_length; i++)
	{
 		//注意:DATA.B[i]该寄存器最多可以存8个字节的数据
...
void CAN1_Rec_Interrupt(uint32_t *RxCODE_T,uint32_t *RxID_T,uint32_t *Length,uint8_t *Buff)
{
	uint32_t j,RxTIMER_T,dummy;
	  ...
	  *RxCODE_T   = CAN_1.MB[0].CS.B.CODE;   //CODE寄存器有4位，用于只是对应MB的状态，未激活/空/满/被覆盖......
	  *RxID_T     = CAN_1.MB[0].ID.B.ID_STD; //报文ID
	  *Length     = CAN_1.MB[0].CS.B.DLC;    //数据长度
	  if(*RxID_T != 0x666 )
	  {
		  for (j=0; j<*Length; j++)
		  	  {
...
void CAN2_Rec_Interrupt(uint32_t *RxCODE_T,uint32_t *RxID_T,uint32_t *Length,uint8_t *Buff)
{
	uint32_t j,RxTIMER_T,dummy;
	  ...
	  *RxCODE_T   = CAN_2.MB[0].CS.B.CODE;   //CODE寄存器有4位，用于只是对应MB的状态，未激活/空/满/被覆盖......
	  *RxID_T     = CAN_2.MB[0].ID.B.ID_STD; //报文ID
	  *Length     = CAN_2.MB[0].CS.B.DLC;    //数据长度
	  if(*RxID_T != 0x666 )
	  {
		  for (j=0; j<*Length; j++)
		  	  {
...
{
	uint8_t	i;
	CAN_1.MB[0].CS.B.IDE = 1;       	 //1:扩展帧    29位
	CAN_1.MB[0].ID.B.ID_STD = 0x666;	 //标准ID
	CAN_1.MB[0].ID.B.ID_EXT = 0;		 //扩展ID
	CAN_1.MB[0].CS.B.RTR = 0;            /* Data frame, not remote Tx request frame */
 	CAN_1.MB[0].CS.B.DLC = Data_length ; //要发送的长度
 	for (i=0; i<Data_length; i++)
	{
	...
	CAN_1.MB[0].CS.B.SRR = 1;     /* Tx frame (not req'd for standard frame)*/
	CAN_1.MB[0].CS.B.CODE =0xC;   //无条件发送一次数据
}
void CAN2_Trans_Message(int ID_Message, uint8_t Data_length, uint8_t TxData[])
{
	uint8_t	i;
	CAN_2.MB[0].CS.B.IDE = 1;       	 //1:扩展帧    29位
	CAN_2.MB[0].ID.B.ID_STD = 0x666;	 //标准ID
	CAN_2.MB[0].ID.B.ID_EXT = 0;		 //扩展ID
	CAN_2.MB[0].CS.B.RTR = 0;            /* Data frame, not remote Tx request frame */
 	CAN_2.MB[0].CS.B.DLC = Data_length ; //要发送的长度
 	for (i=0; i<Data_length; i++)
	{
	...
	CAN_2.MB[0].CS.B.SRR = 1;     /* Tx frame (not req'd for standard frame)*/
	CAN_2.MB[0].CS.B.CODE =0xC;   //无条件发送一次数据
}
